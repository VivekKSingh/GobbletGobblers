...
class GobbletPlayer(game_player.GamePlayer):
    ...
    # state is a TicTacToeState, otherPlayer is a valid game ID.
        ...
        def open3(self, state, adversaryGob):
        s = 0
        if (state.board_value(0,0) != adversaryGob \
                and (state.board_value(1,1) != adversaryGob\
                and (state.board_value(2,2) != adversaryGob:
            s += 1
        if (state.board_value(2,0) != adversaryGob\
                and (state.board_value(1,1) != adversaryGob\
                and (state.board_value(0,2) != adversaryGob:
            s += 1
        
        if (state.board_value(0,0) != adversaryGob\
                and (state.board_value(0,1) != adversaryGob\
                and (state.board_value(0,2) != adversaryGob:
            s += 1
                                         
            
        if (state.board_value(1,0) != adversaryGob\
                and (state.board_value(1,1) != adversaryGob \
                and (state.board_value(1,2) != adversaryGob:
            s += 1
                
        if (state.board_value(2,0) != adversaryGob \
                and (state.board_value(2,1) != adversaryGob \
                and (state.board_value(2,2) != adversaryGob:
            s += 1
        if (state.board_value(0,0) != adversaryGob \
                and (state.board_value(1,0) != adversaryGob\
                and (state.board_value(2,0) != adversaryGob:
            s += 1

        if (state.board_value(0,1) != adversaryGob \
                and (state.board_value(1,1) != adversaryGob \
                and (state.board_value(2,1) != adversaryGob:
            s += 1
        if (state.board_value(0,2) != adversaryGob\
                and (state.board_value(1,2) != adversaryGob\
                and (state.board_value(2,2) != adversaryGob:
            s += 1
	...
	def minimax_search(self, state, h):
		...
		successors = state.successors_moves()
